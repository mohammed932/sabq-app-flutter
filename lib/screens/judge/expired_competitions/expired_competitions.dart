...
class ExpiredCompetitionsScreen extends StatefulWidget {
  @override
  _ExpiredCompetitionsScreenState createState() =>
      _ExpiredCompetitionsScreenState();
}

class _ExpiredCompetitionsScreenState extends State<ExpiredCompetitionsScreen> {
  ...
  init() async {
    ...
    competitionBloc.getExpiredCompetitions();
  ...
  buildCompetitionList(CompetitionBloc state) {
    return state.expiredCompetitions.isNotEmpty
        ? ListView.builder(
            itemCount: state.expiredCompetitions.length,
            padding: EdgeInsets.only(
                bottom: 200.0, left: 15.0, right: 15.0, top: 20.0),
            itemBuilder: (BuildContext context, int index) {
              return GestureDetector(
                  onTap: () => openDetails(state.expiredCompetitions[index]),
                  child: CompetitionCard(
                    competition: state.expiredCompetitions[index],
                  ));
            })
        : EmptyCompetition(
            text: TranslationBase.of(context)
                .getStringLocaledByKey('NO_EXPIRED_COMPETITIONS'),
          );
  }
}
